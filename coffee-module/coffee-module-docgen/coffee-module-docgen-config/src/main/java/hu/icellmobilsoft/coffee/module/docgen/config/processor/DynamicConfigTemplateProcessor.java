 ...
 * {@link DynamicConfigTemplate}, using {@link hu.icellmobilsoft.coffee.module.docgen.config.ConfigDoc} to generate template.
 *
...
public class DynamicConfigTemplateProcessor extends AbstractProcessor {
     ...
     */
    public DynamicConfigTemplateProcessor() {
        super();
    }
    ...
    private void writeDataListSorted(String fileName, List<DocData> lDataList, ConfigDocConfig config) {
        if (CollectionUtils.isNotEmpty(lDataList)) {
            ...
            writeToFile(sortedList, new ConfigDocAsciiDocWriter(config), fileName);
        }
    }
