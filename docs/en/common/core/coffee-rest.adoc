...
By invoking the EvictAction, the cache contents can be cleared.
To achieve this, a REST endpoint must be created in the project, through which the EvictAction::evict method should be called.
As a result, the evict method will be invoked on all beans and config sources implementing the Evictable interface.
