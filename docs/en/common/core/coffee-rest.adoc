...
<3> Determines if the JSON output should be pretty-printed (indented).
<4> Controls whether null values should be serialized.
<5> Specifies the character encoding.
<6> Enables strict I-JSON compliance if set to true.
<7> Custom property naming strategy class for converting Java field names to JSON keys.
<8> Determines the order of properties in the serialized JSON output.
<9> Specifies the date format used in serialization/deserialization, along with the locale.
<10> Defines the Locale to be used for localization during JSON processing.
<11> Specifies custom JsonbAdapter classes for additional control over serialization and deserialization.
<12> Custom JsonbAdapter classes for additional control over JSON processing.
<12> Custom JsonbSerializer classes for serializing specific types.
<13> Custom JsonbDeserializer classes for deserializing specific types.
<14> If set to true, requires parameters for object creation to be present in JSON.
