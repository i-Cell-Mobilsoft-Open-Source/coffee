...
<6> In case of using redis sentinel the master asking node urls in `$(host):$(port)` format. In case of sentinel field is required, otherwise it is forbidden.
<7> The name of the master with which we retrieve the current master node with the following command pattern "redis-cli -h <host> -p <port> SENTINEL get-master-addr-by-name <master>". As there could be more sentinels e.g. "stream" or "cache". In case of sentinel field is required, otherwise it is forbidden.
[NOTE]
====
