...
* If the `ConsumerLifeCycleManager.CONSUMER_COUNTER` less than one, the `ConsumerLifeCycleManager.stop()` will be skipped the `ConsumerLifeCycleManager.SEMAPHORE.acquire();` so in those cases where the container didn't contain any consumer, for e.g. in tests, the shutdown phase won't be stacked.
