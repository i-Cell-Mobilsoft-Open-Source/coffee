...
To close `AbstractPipeline` objects became required with usage of `UnifiedJedis` to avoid running out of redis connections.

==== Migration
On projects should check `redis.clients.jedis.AbstractPipeline` is closed after usage.