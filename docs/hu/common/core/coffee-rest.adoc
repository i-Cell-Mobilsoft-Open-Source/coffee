...
Az EvictAction hívásával kipucolható a cache tartalma, ehhez a projekten létre kell hozni egy REST végpontot, amin keresztül az EvictAction::evict metódust kell meghívni. Ennek hatására az összes Evictable interfészt implementáló beanen, configsource-on meghívódik az evict metódus.
