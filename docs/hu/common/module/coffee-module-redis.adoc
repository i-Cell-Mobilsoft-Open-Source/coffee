...
coffee:
    redis:
        ...
        auth: #<3>
            ...
            sentinel: #<6>
              - sample-sandbox.icellmobilsoft.hu:26379 #default null
            master: stream #<7>
----
...
<6> Redis sentinel használata esetén a master lekérő node url-ek `$(host):$(port)` formátumban. In case of sentinel field is required, otherwise it is forbidden.
<7> A master neve, amivel az aktuális master node (amivel közvetlen kommunikálunk) lekérhető a következő parancs mintával "redis-cli -h <host> -p <port> SENTINEL get-master-addr-by-name <master>". Mivel több sentinel is működhet egymás mellett pl. "stream" vagy "cache". Sentinel esetén kötelező, különben tiltott.
[NOTE]
====
