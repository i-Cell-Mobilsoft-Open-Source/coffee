...
* Amennyiben a `ConsumerLifeCycleManager.CONSUMER_COUNTER` kissebb, mint nulla, a `ConsumerLifeCycleManager.SEMAPHORE.acquire();` hívás nem törtínik meg, így az olyan esetekben, ahol nincs egyetlen consuemr sem (pl tesztek) a leállás nem akad meg.
