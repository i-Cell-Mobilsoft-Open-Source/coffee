...
* ResponseUtil bővítése, getFileResponse esetén a header-ben UTF-8 kódolással is visszaküldjük a filenevet (filename*=UTF-8''...)
...
=== coffee-cdi
* `hu.icellmobilsoft.coffee.cdi.trace.annotation.Traced.LITERAL` típusa megváltozott, hogy használható lehessen `ITraceHandler`-rel,
és inicializálva lett a default értékekkel.
* A `LogContainer` most már limitált darab log üzeneteket tárol. A korlát a `coffee.logger.logContainer.maxSize` konfigurációs kulccsal állítható. (default: `1000`)

==== Migration
Ha a régi `LogContainer` viselkedést szeretnénk visszakapni, a `coffee.logger.logContainer.maxSize` konfigurációs kulcsot kell `2147483647`-re beállítani. De ezt inkább nem javasoljuk.

=== coffee-module-redisstream
`UnifiedJedis` bevezetésével szükségessé vált az `AbstractPipeline` objektumok lezárása, a redis connection-ök elfogyásának megakadályozására.

==== Migration
Projekteken ellenőrizni kell, hogy a `redis.clients.jedis.AbstractPipeline` használat után lezárásra kerüljön.