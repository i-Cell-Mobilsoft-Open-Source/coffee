...
* Java EE 8 -> Jakarta EE 8 átállás - upgrade Jakarta EE 8 API-ra, amely alapvetően visszafele kompatibilis. Átállási munkát
csak abban az esetben eredményez, ha a `jaxb-api` verziója nem volt definiálva projekt szinten. Ekkor azt vagy definiálni kell,
vagy az újonnan behúzott `jakarta.xml.bind-api`-t használni helyette.
