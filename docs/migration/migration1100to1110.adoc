...
=== coffee-cdi
* Deltaspike függőségek megszűntek, átkerültek a coffee-module-localization modulhoz. Ezzel együtt a `Logproducer.getAppLogger()` megváltozott, már nem `DependentProvider`-el tér vissza és `Logproducer.getAppLoggerInstance()` lett a neve.
...
----
...
Instance<AppLogger> logInstance = LogProducer.getAppLoggerInstance();
