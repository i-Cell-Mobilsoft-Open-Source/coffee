...
Refaktorálásra került a `DefaultGeneralExceptionMapper` eddig a `handleException` és a `handleProductionStageException` metődusokban is szerepelt egy sor if-else.
