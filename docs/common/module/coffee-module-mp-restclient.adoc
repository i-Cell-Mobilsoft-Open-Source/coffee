...
Ez alapján a `FaultTypeParser` képes az mp rest client hívósokban a válasz String faultCode-ot enumra parsolni és megfelelő exceptionre mappelni.
...
NOTE:: A felolvasott implementációk között nincs priorizálás, így enum név ütközés esetén a korábban felolvasott
enum osztályra lesz parsolva.
