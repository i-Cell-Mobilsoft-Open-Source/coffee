...
[#common_core_coffee-rest_BaseRestLoggerOptimized]
== BaseRestLoggerOptimized
...
hogy a request és response entity-ket nem a stream-ek másolásával szerzi meg a logolás számára, hanem az entity steam olvasása és írása közben gyűjti azokat.
...
IMPORTANT: ha a `maxEntityLogSize` a `LogSpecifier.NO_LOG`-tól *eltérő* értékre van állítva, akkor a REST endpoint-hoz érkező `application/octet-stream`, `multipart/form-data`, `application/json`, `application/xml` és `text/xml` mediaType-ok esetében csak a kérés első 5000 karaktere íródik ki.
