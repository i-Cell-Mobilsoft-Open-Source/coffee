...
IMPORTANT: `BaseRestLoggerOptimized` használata esetén, ha nincsen megadva a `LogSpecifier` annotáció, akkor `application/octet-stream` és `multipart/form-data` mediaType-ok esetében a kérésben és a válaszban lévő entity első 5000 karaktere íródik csak ki.
